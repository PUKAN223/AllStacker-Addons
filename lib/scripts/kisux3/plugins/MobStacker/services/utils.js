import { EntityIsBabyComponent, EntityLeashableComponent, EntityScaleComponent, system } from "@minecraft/server";
import { getAllEntities } from "../../../../core/utils/EntityManagers";
export function* StackingMob(config) {
    new Promise((resolve) => __awaiter(this, void 0, void 0, function* () {
        try {
            const allEntities = getAllEntities((en) => {
                if (!config.ResetEntities.has(en) &&
                    [...config.MobStackConfig.get("StackMob") || []].some(b => b === en.typeId) &&
                    en.location)
                    return true;
                return false;
            });
...
export function EntityToName(en) {
    return en.typeId.split(":")[1]
        .split("_")
        .map((word) => word.charAt(0).toUpperCase() + word.slice(1))
        .join(" ");
}
export function getEntitiesNearBy(dimension, en, config) {
...
export function getMobColorCode(amount) {
    if (amount >= 1290)
        return "§9";
    if (amount >= 960)
        return "§b";
...
export function spawnEntityClone(en) {
