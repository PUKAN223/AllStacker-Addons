...
import { ItemConvert } from "../../../../core";
import { getAllPlayers } from "../../../../core/utils/PlayerManagers";
export function* StackingItem(config) {
    try {
...
export function* SeeingItem(config) {
    try {
...
export default function getItemNearBy(en, config) {
    ...
    return allEntities.filter((target) => {
        ...
        if (itemStack.hasComponent(ItemEnchantableComponent.componentId) && targetItemStack.hasComponent(ItemEnchantableComponent.componentId)) {
            ...
            for (let i = 0; i < itemEnchants.length; i++) {
                if (itemEnchants[i].type.id !== targetEnchants[i].type.id ||
...
export function ItemsToName(entity) {
...
export function getItemColorCode(amount) {
    if (amount >= 1290)
        return "§9";
    if (amount >= 960)
        return "§b";
...
export function deStackItemStack(config, itemRemovedData) {
    try {
        ...
        if (itemToSpawn > 0) {
            ...
            const enBase = world.getDimension(itemRemovedData.dim).spawnItem(itemStackSpawn, Object.assign(Object.assign({}, itemRemovedData.location), { y: world.getDimension(itemRemovedData.dim).heightRange.max }));
        ...
        system.run(() => {
            ...
            sizeStack.forEach((item) => {
                ...
                const enBase = world.getDimension(itemRemovedData.dim).spawnItem(ItemConvert.JsonToItem(itemStackSpawn), Object.assign(Object.assign({}, itemRemovedData.location), { y: itemRemovedData.location.y + 100 }));
...
export function getSizeStack(current, amount, maxStack) {
