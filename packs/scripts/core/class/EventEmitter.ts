...
type AnyEventKey =
  | PrefixedAfterKeys
  | PrefixedBeforeKeys
  | PrefixedSystemAfterKeys
  | PrefixedSystemBeforeKeys
  | `after:tick`
  | `custom:${string}`;
...
/**
 * Central event system for the plugin framework.
 * Handles event subscription and emission for both global and plugin-specific events.
 */
class KXEvents {
      ...
      for (const [key, pluginEvents] of this.events.entries()) {
        ...
        for (const cb of callbacks) {
          try {
            ...
            console.error(
              `[KXEvents] Error in event "${eventName}" from ${key}:`,
              (err as Error).stack,
            );
          }
        }
