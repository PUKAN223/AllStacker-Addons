...
class DynamicSource {
  ...
  delete(key) {
    this.sDP.call(this.source, key, undefined);
    return true;
  }
  /**@returns {boolean}  */
  isValid() {
    return this.source === world || isValidEntity.call(this.source);
  }
}
///////////////////////////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////   DYNAMIC DATABASE
...
class DynamicDatabase extends Map {
  ...
  constructor(source, id, kind, parser) {
    ...
    const PRE = `${kind}${DYNAMIC_DB_PREFIX}${id}${DYNAMIC_DB_PREFIX}`,
      LENGTH = PRE.length,
      SOURCE = this._source,
      PARSE = parser.parse;
...
class DynamicWrapper {
  ...
  clear() {
    for (const k of this.__getKeys()) this._source.set(k, undefined);
  }
  /**
   ...
   */
  get(key) {
    const a = this._source.get(this._prefix + key);
    typeof a === "string" ? this._PARSE(a) : a;
  }
  /**
   * @returns boolean indicating whether an element with the specified key exists or not.
   */
  has(key) {
    return this._source.get(this._prefix + key) !== undefined;
  }
  /**
  ...
  *entries() {
   ...
   */
  *keys() {
    for (const k of this.__getKeys()) yield k.substring(this._prefixLength);
  }
  /**
   * Returns an iterable of values in the map
   */
  *values() {
    for (const k of this.__getKeys()) yield this._PARSE(this._source.get(k));
  }
  *__getKeys() {
    for (const K of this._source.getIds()) {
      if (K.startsWith(this._prefix)) yield K;
    }
  }
}
...
function getRootTable() {
  ...
  return RootTable = world.getDynamicProperty(ROOT_KEY)
    ? DATABASE_MANAGER.deserialize(ROOT_KEY, new DynamicSource(world))
    : (() => {
...
const Serializer = {
  isSerializable(object) {
    return object[PARSER_SYMBOL] != undefined;
  },
  ...
  setSerializableClass(construct, kind, serializer, deserializer) {
    if (
      typeof serializer !== "function" || typeof deserializer !== "function"
    ) throw new TypeError("Serializer or deserializer is not a function");
...
export const registryAPISerializers = () => {
  ...
  if (ItemStackSupportLevel.durability) {
    ItemComponentSerializers[ItemStackSupportLevel.durability.componentId] =
      function (component) {
        return component.damage;
      };
    ItemComponentDeserializers[ItemStackSupportLevel.durability.componentId] =
      function (component, v) {
        component.damage = v;
      };
  }
  if (ItemStackSupportLevel.enchantable) {
    ItemComponentSerializers[ItemStackSupportLevel.enchantable.componentId] =
      function (component) {
        return component.getEnchantments().map((e) => ({
          t: e.type.id,
          l: e.level,
        }));
      };
    ItemComponentDeserializers[ItemStackSupportLevel.enchantable.componentId] =
      function (component, v) {
        component.addEnchantments(v.map((e) => ({ type: e.t, level: e.l })));
      };
  }
  ...
  if (BlockTypes) {
    Serializer.setSerializableClass(
      BlockType,
      APISerializableKinds.BlockType,
      function* (n) {
        yield n.id;
      },
      function (n) {
        for (const a of n) return BlockTypes.get(a);
      },
    );
  }
  if (EntityTypes) {
    Serializer.setSerializableClass(
      EntityType,
      APISerializableKinds.EntityType,
      function* (n) {
        yield n.id;
      },
      function (n) {
        for (const a of n) return EntityTypes.get(a);
      },
    );
  }
  if (ItemTypes) {
    Serializer.setSerializableClass(
      ItemType,
      APISerializableKinds.ItemType,
      function* (n) {
        yield n.id;
      },
      function (n) {
        for (const a of n) return ItemTypes.get(a);
      },
    );
  }
  ...
  if ("type" in BlockPermutation.prototype) {
    Serializer.setSerializableClass(
      BlockPermutation,
      APISerializableKinds.BlockPermutation,
      function* (n) {
        yield n.type.id;
  ...
  Serializer.setSerializableClass(
    ItemStack,
    APISerializableKinds.ItemStack,
    function* (n) {
      const components = ItemStackSupportLevel.components
        ? [...n.getComponents()].filter((e) =>
          e && (e.typeId in ItemComponentSerializers)
        )
        : [];
      const canPlaceOn = ItemStackSupportLevel.canPlaceOn
        ? n.getCanPlaceOn()
        : [];
      const canDestroy = ItemStackSupportLevel.canDestory
        ? n.getCanDestroy()
        : [];
      const dynamicProperties = ItemStackSupportLevel.dynamicProperties
        ? n.getDynamicPropertyIds()
        : [];
      ...
      for (const com of components) {
        yield JSON.stringify([
          com.typeId,
          ItemComponentSerializers[com.typeId](com),
        ]);
      }
      yield* canPlaceOn;
      yield* canDestroy;
      for (const k of dynamicProperties) {
        ...
        if ((data.length + k.length) > TABLE_STRING_LENGTH) {
          throw new TypeError(
            `Dynamic property '${k}' of this item is too large'${data.length}'`,
          );
        }
  ...
  if (Vector) {
    Serializer.setSerializableClass(
      Vector,
      APISerializableKinds.Vector,
      function (s) {
        const { x, y, z } = s;
        return Object.Serialize({ x, y, z });
      },
      function (n) {
        const { x, y, z } = Object.Deserialize(n);
