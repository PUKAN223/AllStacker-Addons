...
import { KXEvents } from "../class/EventEmitter.ts";
...
export function initializeEvents() {
  ...
  afterEvents.forEach((event) => {
    world.afterEvents[event as keyof typeof world.afterEvents].subscribe(
      (ev) => {
  ...
  beforeEvents.forEach((event) => {
    world.beforeEvents[event as keyof typeof world.beforeEvents].subscribe(
      (ev) => {
  ...
  systemAfterEvents.forEach((event) => {
    system.afterEvents[event as keyof typeof system.afterEvents].subscribe(
      (ev) => {
        KXEvents.emit(null, `after:${event}`, ev);
      },
    );
  ...
  systemBeforeEvents.forEach((event) => {
    system.beforeEvents[event as keyof typeof system.beforeEvents].subscribe(
      (ev) => {
        KXEvents.emit(null, `before:${event}`, ev);
      },
    );
