...
/**
 * ItemStacker plugin - Handles stacking of dropped items in the world.
 * Combines similar items within a configurable radius to optimize performance.
 */
class ItemStacker extends PluginBase {
  ...
  public override addConfig(
    ...
    const optionsConfig = {
      "Stacking Settings": () => {
        const unStackPage = new IActionForm(
          LanguageContext.getTranslation("allstacker.title.unstacked", pl),
          LanguageContext.getTranslation("allstacker.body.unstacked", pl),
        );
        unStackPage.addDivider();
        unStackPage.addLabel(
          LanguageContext.getTranslation("allstacker.label.unstacked", pl),
        );

        unStackPage.addButton(
          LanguageContext.getTranslation("allstacker.button.add_unstacked", pl),
          ...
          () => {
            ...
            const itemSelectForm = new IActionForm(
              LanguageContext.getTranslation(
                "allstacker.title.select_item",
                pl,
              ),
              LanguageContext.getTranslation("allstacker.body.select_item", pl),
            );
            itemSelectForm.addDivider();
            Object.entries(itemList).forEach(([name, item]) => {
              if (!item || !item.typeId) return;
              if (
                this.config.ItemStackConfig!.get("UnStackItem")?.includes(
                  item.typeId,
                )
              ) return;
              itemSelectForm.addButton(name, "", () => {
                const unStackItems =
                  this.config.ItemStackConfig!.get("UnStackItem") || [];
        ...
        unStackPage.addButton(
          ...
          "textures/ui/icon_book_writable",
          () => {
            const unStackItems: string[] =
              this.config.ItemStackConfig!.get("UnStackItem") || [];
            const removeItemForm = new IActionForm(
              LanguageContext.getTranslation(
        ...
        unStackPage.addButton(
          ...
          () => {
            const unStackItems: string[] =
              this.config.ItemStackConfig!.get("UnStackItem") || [];
            const viewItemsForm = new IActionForm(
              LanguageContext.getTranslation(
      ...
      "Advanced Settings": () => {
        const advandSetting = new IModalForm(
          LanguageContext.getTranslation(
        ...
        const isEnable =
          PluginLoader.find((plugin) => plugin.name === this.name)?.setting
            .enabled || false;
        const RadiusSeeing = this.config.ItemStackConfig!.get("RadiusSeeing") ||
          10;
        const RadiusCombine =
          this.config.ItemStackConfig!.get("RadiusCombine") || 15;
        const DisplayText = this.config.ItemStackConfig!.get("DisplayText") ||
          "§7§c§l%a §r%n§r";
        const FastModeStacking =
          this.config.ItemStackConfig!.get("FastModeStacking") || false;
        ...
        advandSetting.addToggle(
          LanguageContext.getTranslation("allstacker.toggle.itemstack", pl),
          isEnable,
        );
        advandSetting.addSlider(
          LanguageContext.getTranslation("allstacker.slider.radius_seeing", pl),
          1,
          50,
          1,
          RadiusSeeing,
        ...
        advandSetting.addCallback((values, canceled) => {
          if (canceled) return;
          const isEnable = values[2] as boolean;
          const radiusSeeing = values[3];
          const radiusCombine = values[4];
          const displayText = values[6] as string;
          const fastModeStacking = values[5];

          const oldEnable = PluginLoader.find((plugin) =>
            plugin.name === this.name
          )?.setting.enabled || false;
          const oldRadiusSeeing =
            this.config.ItemStackConfig!.get("RadiusSeeing") || 10;
          const oldDisplayText =
            this.config.ItemStackConfig!.get("DisplayText") ||
            "§7§c§l%a §r%n§r";
          const oldRadiusCombine =
            this.config.ItemStackConfig!.get("RadiusCombine") || 15;
          const oldFastModeStacking =
            this.config.ItemStackConfig!.get("FastModeStacking") || false;
          ...
          if (
            oldRadiusCombine !== radiusCombine && radiusCombine !== undefined
          ) {
            pl.sendMessage(
              LanguageContext.getTranslation(
          ...
          if (
            fastModeStacking !== oldFastModeStacking &&
            fastModeStacking !== undefined
          ) {
            this.config.ItemStackConfig!.set(
              "FastModeStacking",
              fastModeStacking,
    ...
    const configUi = new IActionForm(
      LanguageContext.getTranslation("allstacker.title.itemstacker", pl),
      LanguageContext.getTranslation("allstacker.body.itemstacker", pl),
    );
    configUi.addDivider();
    configUi.addLabel(
      LanguageContext.getTranslation(
    ...
    configUi.addButton(
      LanguageContext.getTranslation("allstacker.button.stacking_settings", pl),
  ...
  private initializeConfig(): void {
    this.config = this.getConfig() as unknown as IConfigItemStacker;
    ...
    const UnStackItem =
      this.config.ItemStackConfig.get("UnStackItem") as string[] || [];
    const DisplayText = this.config.ItemStackConfig.get("DisplayText") ||
      "§7§c§l%a §r%n§r";
    const RadiusSeeing = this.config.ItemStackConfig.get("RadiusSeeing") || 10;
    const RadiusCombine = this.config.ItemStackConfig.get("RadiusCombine") ||
      15;
