import { ItemComponentUseEvent, ItemStack, Player, StartupEvent, system, world, WorldLoadAfterEvent } from "npm:@minecraft/server@2.3.0";
  ...
  public showConfig(pl: Player): boolean {
    ...
    pluginSettingList.forEach((plugin) => {
      ...
      if (isHasConfig) {
        ...
        pluginListPage.addButton(plugin.name + `\n[${plugin.setting.enabled ? `${LanguageContext.getTranslation("allstacker.label.plugin.enabled", pl)}` : `${LanguageContext.getTranslation("allstacker.label.plugin.disabled", pl)}`}]`, pluginIcon as string, () => {
