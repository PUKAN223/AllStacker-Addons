import { Entity, EntityDamageCause, EntityDieAfterEvent, EntityEquippableComponent, EntityProjectileComponent, EntityRemoveBeforeEvent, EquipmentSlot, Player, PlayerInteractWithEntityBeforeEvent, system, world, WorldLoadAfterEvent } from "npm:@minecraft/server@2.3.0";
import { JsonDatabase, KXEvents, PageBuilder, PluginBase } from "../../../core/index.ts";
import { EntityToName, getMobColorCode, spawnEntityClone, StackingMob } from "./services/utils.ts";
...
class MobStacker extends PluginBase {
  ...
  public onEntityDie(ev: EntityDieAfterEvent) {
    ...
    const MobDeathMode = this.config.MobStackConfig.get("MobDeathMode") as string || "All";
    if (MobDeathMode === "All") {
  ...
  public override addConfig(pl: Player, page: PageBuilder, showUI: boolean = true): boolean {
    ...
    configUI.addButton(LanguageContext.getTranslation("allstacker.button.mobstacker_settings", pl), "textures/blocks/build_allow", () => {
      ...
      stackedUI.addButton(LanguageContext.getTranslation("allstacker.button.add_stacked_mobs", pl), "textures/ui/icon_book_writable", () => {
        ...
        const mobStackList = this.config.MobStackConfig.get("StackMob") as string[] || [];
    ...
    configUI.addButton(LanguageContext.getTranslation("allstacker.button.advanced_settings", pl), "textures/ui/advanced_glyph_color", () => {
      ...
      advancedSettingsUI.addToggle(LanguageContext.getTranslation("allstacker.toggle.mobstacker", pl), PluginLoader.find((pl) => pl.name === this.name)?.setting.enabled || false);
      ...
      advancedSettingsUI.addSlider(LanguageContext.getTranslation("allstacker.slider.radius_stacking", pl), 1, 100, 1, this.config.MobStackConfig.get("RadiusStacking") as number || 10);
