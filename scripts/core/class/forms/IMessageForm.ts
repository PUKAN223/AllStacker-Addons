...
import { IMessageFormButton } from "../../types/IMessageForm/Elements/Button";
...
class IMessageForm {
     ...
     */
    public async showWithCallback(player: Player, callback: (selection: number | undefined, canceled: boolean) => void): Promise<void> {
        try {
     ...
     */
    public static createConfirmation(title: string, body: string, onConfirm?: () => void, onCancel?: () => void): IMessageForm {
        return new IMessageForm(title, body)
            .setButton1("Cancel", onCancel)
