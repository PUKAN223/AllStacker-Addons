...
function deStackItemStack(config, itemRemovedData) {
  try {
    ...
    if (itemToSpawn > 0) {
      ...
      const enBase = world5.getDimension(itemRemovedData.dim).spawnItem(itemStackSpawn, { ...itemRemovedData.location, y: itemRemovedData.location.y + 100 });
...
// scripts/kisux3/plugins/ItemStacker/index.ts
var itemName = (item) => {
  return item.split(":")[1].split("_").map((word) => word.charAt(0).toUpperCase() + word.slice(1)).join(" ");
};
...
import { EntityDamageCause, EntityEquippableComponent, EquipmentSlot, system as system7, world as world10 } from "@minecraft/server";
