// scripts/kisux3/plugins/ItemStacker/index.ts
import { system as system5, world as world8 } from "@minecraft/server";

// scripts/core/events/index.ts
import { system, world } from "@minecraft/server";

// scripts/core/class/PluginManagers.ts
...
// scripts/core/class/EventEmitter.ts
var KXEvents = class {
  static {
  ...
  static emit(plugin, eventName, data) {
    if (plugin === null) {
      ...
      for (const [key2, pluginEvents2] of this.events.entries()) {
        if (key2 === "__global__")
          continue;
        if (!PluginManager.isEnabled(key2))
          continue;
...
// scripts/core/class/ItemConverter.ts
...
// scripts/core/class/forms/IActionForm.ts
...
// scripts/core/class/forms/IMessageForm.ts
...
// scripts/core/class/forms/IModalForm.ts
...
// scripts/core/class/PageBuilders.ts
var PageBuilderData = {};
...
// scripts/core/class/PluginBase.ts
...
// scripts/core/database/Database.js
...
// scripts/kisux3/plugins/ItemStacker/services/utils.ts
...
// scripts/core/utils/EntityManagers.ts
...
// scripts/core/utils/PlayerManagers.ts
...
function* StackingItem(config) {
  try {
    ...
    const CombineItemStack = (en) => {
      ...
      let totalAmount = 0;
      if (!(item.nameTag || item.typeId.includes("potion") || item.typeId.includes("shulker_box") || item.typeId.includes("bundle") || item.typeId.includes("bed") || [...UnStackItem].some((x) => item.typeId.includes(x)))) {
        ...
        for (const target of itemNearBy) {
          ...
          if (config.ItemListStack.has(target))
            config.ItemListStack.delete(target);
...
function getItemNearBy(en, config) {
  ...
  const allEntities = getAllEntities((x) => {
    if (x.dimension !== en.dimension)
      return false;
    if (x.typeId !== "minecraft:item")
      return false;
    if (Vector3Utils.distance(en.location, x.location) > radius)
      return false;
    return true;
  });
  ...
  return allEntities.filter((target) => {
    if (!en.isValid || !target.isValid)
      return false;
    if (target.id === en.id)
      return false;
    const jsonTarget = ItemConvert.ItemToJson(target.getComponent("item").itemStack);
    jsonTarget.amount = 0;
    if (JSON.stringify(jsonItem) !== JSON.stringify(jsonTarget))
      return false;
    const targetItemStack = target.getComponent("item").itemStack;
    if ([...UnStackItem].some((x) => x == targetItemStack.typeId))
      return false;
    if (targetItemStack.nameTag)
      return false;
    if (!config.ItemStackData.has(target.id))
      return false;
    if (itemStack.getLore().join(",") !== targetItemStack.getLore().join(","))
      return false;
    if (itemStack.typeId !== targetItemStack.typeId)
      return false;
    if (itemStack.getTags().join(",") !== targetItemStack.getTags().join(","))
      return false;
    if (itemStack.hasComponent("minecraft:potion") || targetItemStack.hasComponent("minecraft:potion"))
      return false;
    if (itemStack.hasComponent("minecraft:book") || targetItemStack.hasComponent("minecraft:book"))
      return false;
    if (itemStack.hasComponent("minecraft:inventory") || targetItemStack.hasComponent("minecraft:inventory"))
      return false;
    ...
    if (itemStack.hasComponent(ItemEnchantableComponent2.componentId) && targetItemStack.hasComponent(ItemEnchantableComponent2.componentId)) {
      ...
      if (itemEnchants.length !== targetEnchants.length)
        return false;
...
// scripts/kisux3/plugins/ConfigMenu/index.ts
import { system as system4, world as world7 } from "@minecraft/server";

// scripts/kisux3/configs/Lang.ts
import { world as world6 } from "@minecraft/server";

// scripts/core/class/LangguageContext.ts
...
KXEvents.on(null, "after:worldLoad", (ev) => {
  LanguageContext2.setLanguage("en", {
    "allstacker.title.configmenu": (pl) => "\xA78All Stackers Settings",
    "allstacker.body.configmenu": (pl) => `Hello, \xA7e${pl.name}\xA7r!

This is the configuration menu.
You can manage settings here.`,
    "allstacker.button.language": (pl) => "\xA73Language",
    "allstacker.title.language": (pl) => "\xA78Language Settings",
    "allstacker.body.language": (pl) => "Select your preferred language.",
    "allstacker.message.language.set.english": (pl) => `\xA77[All Stacker] \xA7rLanguage set to \xA7aEnglish\xA7r.`,
    "allstacker.message.language.set.thai": (pl) => `\xA77[All Stacker] \xA7rLanguage set to \xA7a\u0E44\u0E17\u0E22\xA7r.`,
    "allstacker.button.back": (pl) => `\xA7cBack`,
    "allstacker.label.plugin.list": (pl) => `\xA77Plugins`,
    "allstacker.label.plugin.enabled": (pl) => `\xA72Enabled\xA7r`,
    "allstacker.label.plugin.disabled": (pl) => `\xA7cDisabled\xA7r`,
    // ItemStacker Main Settings
    "allstacker.title.itemstacker": (pl) => "\xA78Item Stackers Settings",
    "allstacker.body.itemstacker": (pl) => "Adjust the settings for item stacking.",
    "allstacker.label.itemstacker.description": (pl) => "\xA7aCan\xA77 add, remove, view \xA7cunstacked items\xA7r.",
    "allstacker.button.stacking_settings": (pl) => "Stacking Settings",
    "allstacker.button.advanced_settings": (pl) => "Advanced Settings",
    "allstacker.label.advanced.description": (pl) => "\xA7aCan\xA77 \xA72on\xA77/\xA7coff\xA7r and adjust the radius for seeing items, display text, and more.",
    // Unstacked Items
    "allstacker.title.unstacked": (pl) => "\xA78Unstacked Items",
    "allstacker.body.unstacked": (pl) => "Manage the items that are not stacked.",
    "allstacker.label.unstacked": (pl) => "\xA7aCan\xA77 add, remove, view \xA7cunstacked items\xA7r.",
    ...
    // Add Unstacked Item
    "allstacker.title.select_item": (pl) => "\xA78Select Item",
    "allstacker.body.select_item": (pl) => "Select an item in your inventory to unstack.",
    "allstacker.message.unstacked.added": (pl) => `\xA7aAdded\xA7r %name to unstacked items.`,
    // Remove Unstacked Item
    "allstacker.title.remove_unstacked": (pl) => "\xA78Remove Unstacked Item",
    "allstacker.body.remove_unstacked": (pl) => "Select an item to remove from unstacked items.",
    "allstacker.message.unstacked.removed": (pl) => `\xA7cRemoved\xA7r %name from unstacked items.`,
    // View Unstacked Items
    "allstacker.title.unstacked_items": (pl) => "\xA78Unstacked Items",
    "allstacker.body.unstacked_items": (pl) => "List of items that are not stacked.",
    "allstacker.label.no_unstacked_items": (pl) => "\xA7cNo unstacked items found.",
    // Advanced Settings
    "allstacker.title.advanced_settings": (pl) => "\xA78Advanced Settings",
    ...
    "allstacker.toggle.itemstack": (pl) => "\xA7cOFF\xA77/\xA7aON \xA7rItemStack\xA7r",
    "allstacker.slider.radius_seeing": (pl) => "\xA77Radius to seeing items\xA7r",
    "allstacker.slider.radius_combine": (pl) => "\xA77Radius to combine items\xA7r",
    "allstacker.textfield.display_text": (pl) => "\xA77Display Text\xA7r\n %%a\xA77 - show amount\xA7r\n %%n \xA77- show name\xA7r\n %%m \xA77- show minutes\xA7r\n %%s \xA77- show seconds\xA7r\n %%l \xA77- new line\xA7r",
    "allstacker.textfield.display_text.placeholder": (pl) => "Customize the text displayed for stacked items.",
    // Messages
    "allstacker.message.display_text.changed": (pl) => "\xA7aDisplay text changed to: %value",
    "allstacker.message.radius_seeing.changed": (pl) => "\xA7aRadius to seeing items\xA7r changed to: %value",
    "allstacker.message.radius_combine.changed": (pl) => "\xA7aRadius to combine items\xA7r changed to: %value",
    "allstacker.message.plugin.enabled": (pl) => "\xA7aItemStacker plugin is now enabled!",
    "allstacker.message.plugin.disabled": (pl) => "\xA7aItemStacker plugin is now disabled!",
    // MobStacker Main Settings
    "allstacker.title.mobstacker": (pl) => "\xA78Mob Stacker Settings",
    "allstacker.body.mobstacker": (pl) => "Configure the Mob Stacker plugin.",
    "allstacker.label.mobstacker.description": (pl) => "\xA7aCan\xA77 add, remove, view \xA7bstacked mobs\xA7r.",
    "allstacker.button.mobstacker_settings": (pl) => "Stacking Settings",
    "allstacker.label.mobstacker.advanced.description": (pl) => "\xA7aCan\xA77 \xA72on\xA77/\xA7coff\xA7r and adjust the radius of stacking, display text of stacked mobs, and more.",
    // MobStacker Stacking Settings
    "allstacker.title.mob_stacking_settings": (pl) => "\xA78Stacking Settings",
    ...
    // Add Stacked Mobs
    "allstacker.title.add_stacked_mobs": (pl) => "\xA78Add Stacked Mobs",
    "allstacker.body.add_stacked_mobs": (pl) => "Select the mobs you want to stack within a radius of 10 blocks.",
    "allstacker.label.no_stackable_mobs": (pl) => "\xA7cNo stackable mobs found in the radius.",
    "allstacker.message.mob.added": (pl) => "\xA7aAdded %name to the stackable mobs.",
    // Remove Stacked Mobs
    "allstacker.title.remove_stacked_mobs": (pl) => "\xA78Remove Stacked Mobs",
    "allstacker.body.remove_stacked_mobs": (pl) => "Select the mobs you want to remove from stacking.",
    "allstacker.label.no_stacked_mobs": (pl) => "\xA7cNo stackable mobs found.",
    "allstacker.message.mob.removed": (pl) => "\xA7aRemoved %name from the stackable mobs.",
    // View Stacked Mobs
    "allstacker.title.view_stacked_mobs": (pl) => "\xA78View Stacked Mobs",
    "allstacker.body.view_stacked_mobs": (pl) => "List of currently stackable mobs.",
    // MobStacker Advanced Settings
    "allstacker.title.mob_advanced_settings": (pl) => "\xA78Advanced Settings",
    ...
    "allstacker.toggle.mobstacker": (pl) => "\xA7cOFF\xA77/\xA7aON\xA7f MobStacker",
    "allstacker.dropdown.mob_death_mode": (pl) => "\xA77Mob Death Mode",
    "allstacker.slider.radius_stacking": (pl) => "\xA77Radius to stacking near mobs",
    "allstacker.textfield.mob_display_text": (pl) => "\xA77Display Text\n \xA7r%%a \xA77- show amount\n\xA7r %%n \xA77- show name\n \xA7r%%l \xA77- new line",
    "allstacker.textfield.mob_display_text.placeholder": (pl) => "Enter the display text for stacked mobs",
    "allstacker.button.save_changes": (pl) => "\xA78Save Changes",
    // MobStacker Messages
    "allstacker.message.mob_death_mode.changed": (pl) => "\xA7aUpdated mob death mode to %value.",
    "allstacker.message.stacking_radius.changed": (pl) => "\xA7aUpdated stacking radius to %value blocks.",
    "allstacker.message.mob_display_text.changed": (pl) => "\xA7aUpdated display text to: %value",
    "allstacker.message.mobstacker.enabled": (pl) => "\xA7aMob Stacker plugin is now enabled.",
    "allstacker.message.mobstacker.disabled": (pl) => "\xA7aMob Stacker plugin is now disabled."
  });
  LanguageContext2.setLanguage("th", {
    "allstacker.title.configmenu": (pl) => "\xA78\u0E01\u0E32\u0E23\u0E15\u0E31\u0E49\u0E07\u0E04\u0E48\u0E32\u0E17\u0E31\u0E49\u0E07\u0E2B\u0E21\u0E14",
    "allstacker.body.configmenu": (pl) => `\u0E2A\u0E27\u0E31\u0E2A\u0E14\u0E35, \xA7e${pl.name}\xA7r!

\u0E19\u0E35\u0E48\u0E04\u0E37\u0E2D\u0E40\u0E21\u0E19\u0E39\u0E01\u0E32\u0E23\u0E15\u0E31\u0E49\u0E07\u0E04\u0E48\u0E32.
\u0E04\u0E38\u0E13\u0E2A\u0E32\u0E21\u0E32\u0E23\u0E16\u0E08\u0E31\u0E14\u0E01\u0E32\u0E23\u0E01\u0E32\u0E23\u0E15\u0E31\u0E49\u0E07\u0E04\u0E48\u0E32\u0E44\u0E14\u0E49\u0E17\u0E35\u0E48\u0E19\u0E35\u0E48.`,
    "allstacker.button.language": (pl) => "\xA73\u0E20\u0E32\u0E29\u0E32",
    "allstacker.title.language": (pl) => "\xA78\u0E01\u0E32\u0E23\u0E15\u0E31\u0E49\u0E07\u0E04\u0E48\u0E32\u0E20\u0E32\u0E29\u0E32",
    "allstacker.body.language": (pl) => "\u0E40\u0E25\u0E37\u0E2D\u0E01\u0E20\u0E32\u0E29\u0E32\u0E17\u0E35\u0E48\u0E04\u0E38\u0E13\u0E15\u0E49\u0E2D\u0E07\u0E01\u0E32\u0E23.",
    "allstacker.message.language.set.english": (pl) => `\xA77[All Stacker] \xA7r\u0E40\u0E1B\u0E25\u0E35\u0E48\u0E22\u0E19\u0E20\u0E32\u0E29\u0E32\u0E40\u0E1B\u0E47\u0E19 \xA7aEnglish\xA7r.`,
    "allstacker.message.language.set.thai": (pl) => `\xA77[All Stacker] \xA7r\u0E40\u0E1B\u0E25\u0E35\u0E48\u0E22\u0E19\u0E20\u0E32\u0E29\u0E32\u0E40\u0E1B\u0E47\u0E19 \xA7a\u0E44\u0E17\u0E22\xA7r.`,
    "allstacker.button.back": (pl) => `\xA7c\u0E01\u0E25\u0E31\u0E1A`,
    "allstacker.label.plugin.list": (pl) => `\xA77\u0E1B\u0E25\u0E31\u0E4A\u0E01\u0E2D\u0E34\u0E19`,
    "allstacker.label.plugin.enabled": (pl) => `\xA72\u0E40\u0E1B\u0E34\u0E14\u0E43\u0E0A\u0E49\u0E07\u0E32\u0E19\xA7r`,
    "allstacker.label.plugin.disabled": (pl) => `\xA7c\u0E1B\u0E34\u0E14\u0E43\u0E0A\u0E49\u0E07\u0E32\u0E19\xA7r`,
    // ItemStacker Main Settings
    "allstacker.title.itemstacker": (pl) => "\xA78\u0E01\u0E32\u0E23\u0E15\u0E31\u0E49\u0E07\u0E04\u0E48\u0E32\u0E01\u0E32\u0E23\u0E23\u0E27\u0E21\u0E44\u0E2D\u0E40\u0E17\u0E47\u0E21",
    "allstacker.body.itemstacker": (pl) => "\u0E1B\u0E23\u0E31\u0E1A\u0E01\u0E32\u0E23\u0E15\u0E31\u0E49\u0E07\u0E04\u0E48\u0E32\u0E2A\u0E33\u0E2B\u0E23\u0E31\u0E1A\u0E01\u0E32\u0E23\u0E23\u0E27\u0E21\u0E44\u0E2D\u0E40\u0E17\u0E47\u0E21.",
    "allstacker.label.itemstacker.description": (pl) => "\xA7a\u0E2A\u0E32\u0E21\u0E32\u0E23\u0E16\xA77 \u0E40\u0E1E\u0E34\u0E48\u0E21, \u0E25\u0E1A, \u0E14\u0E39 \xA7c\u0E44\u0E2D\u0E40\u0E17\u0E47\u0E21\u0E17\u0E35\u0E48\u0E44\u0E21\u0E48\u0E23\u0E27\u0E21\xA7r.",
    "allstacker.button.stacking_settings": (pl) => "\u0E01\u0E32\u0E23\u0E15\u0E31\u0E49\u0E07\u0E04\u0E48\u0E32\u0E01\u0E32\u0E23\u0E23\u0E27\u0E21",
    "allstacker.button.advanced_settings": (pl) => "\u0E01\u0E32\u0E23\u0E15\u0E31\u0E49\u0E07\u0E04\u0E48\u0E32\u0E02\u0E31\u0E49\u0E19\u0E2A\u0E39\u0E07",
    "allstacker.label.advanced.description": (pl) => "\xA7a\u0E2A\u0E32\u0E21\u0E32\u0E23\u0E16\xA77 \xA72\u0E40\u0E1B\u0E34\u0E14\xA77/\xA7c\u0E1B\u0E34\u0E14\xA7r \u0E41\u0E25\u0E30\u0E1B\u0E23\u0E31\u0E1A\u0E23\u0E30\u0E22\u0E30\u0E01\u0E32\u0E23\u0E21\u0E2D\u0E07\u0E40\u0E2B\u0E47\u0E19\u0E44\u0E2D\u0E40\u0E17\u0E47\u0E21, \u0E02\u0E49\u0E2D\u0E04\u0E27\u0E32\u0E21\u0E41\u0E2A\u0E14\u0E07\u0E1C\u0E25 \u0E41\u0E25\u0E30\u0E2D\u0E37\u0E48\u0E19\u0E46",
    // Unstacked Items
    "allstacker.title.unstacked": (pl) => "\xA78\u0E23\u0E32\u0E22\u0E01\u0E32\u0E23\u0E17\u0E35\u0E48\u0E44\u0E21\u0E48\u0E16\u0E39\u0E01\u0E08\u0E31\u0E14\u0E40\u0E23\u0E35\u0E22\u0E07",
    "allstacker.body.unstacked": (pl) => "\u0E08\u0E31\u0E14\u0E01\u0E32\u0E23\u0E23\u0E32\u0E22\u0E01\u0E32\u0E23\u0E17\u0E35\u0E48\u0E44\u0E21\u0E48\u0E16\u0E39\u0E01\u0E08\u0E31\u0E14\u0E40\u0E23\u0E35\u0E22\u0E07.",
    "allstacker.label.unstacked": (pl) => "\xA7a\u0E2A\u0E32\u0E21\u0E32\u0E23\u0E16\xA77 \u0E40\u0E1E\u0E34\u0E48\u0E21, \u0E25\u0E1A, \u0E14\u0E39 \xA7c\u0E23\u0E32\u0E22\u0E01\u0E32\u0E23\u0E17\u0E35\u0E48\u0E44\u0E21\u0E48\u0E16\u0E39\u0E01\u0E08\u0E31\u0E14\u0E40\u0E23\u0E35\u0E22\u0E07\xA7r.",
    ...
    // Add Unstacked Item
    "allstacker.title.select_item": (pl) => "\xA78\u0E40\u0E25\u0E37\u0E2D\u0E01\u0E44\u0E2D\u0E40\u0E17\u0E47\u0E21",
    "allstacker.body.select_item": (pl) => "\u0E40\u0E25\u0E37\u0E2D\u0E01\u0E44\u0E2D\u0E40\u0E17\u0E47\u0E21\u0E43\u0E19\u0E0A\u0E48\u0E2D\u0E07\u0E40\u0E01\u0E47\u0E1A\u0E02\u0E2D\u0E07\u0E02\u0E2D\u0E07\u0E04\u0E38\u0E13\u0E40\u0E1E\u0E37\u0E48\u0E2D\u0E44\u0E21\u0E48\u0E43\u0E2B\u0E49\u0E16\u0E39\u0E01\u0E08\u0E31\u0E14\u0E40\u0E23\u0E35\u0E22\u0E07.",
    "allstacker.message.unstacked.added": (pl) => `\xA7a\u0E40\u0E1E\u0E34\u0E48\u0E21\xA7r %name \u0E25\u0E07\u0E43\u0E19\u0E23\u0E32\u0E22\u0E01\u0E32\u0E23\u0E17\u0E35\u0E48\u0E44\u0E21\u0E48\u0E16\u0E39\u0E01\u0E08\u0E31\u0E14\u0E40\u0E23\u0E35\u0E22\u0E07.`,
    // Remove Unstacked Item
    "allstacker.title.remove_unstacked": (pl) => "\xA78\u0E25\u0E1A\u0E23\u0E32\u0E22\u0E01\u0E32\u0E23\u0E17\u0E35\u0E48\u0E44\u0E21\u0E48\u0E16\u0E39\u0E01\u0E08\u0E31\u0E14\u0E40\u0E23\u0E35\u0E22\u0E07",
    "allstacker.body.remove_unstacked": (pl) => "\u0E40\u0E25\u0E37\u0E2D\u0E01\u0E44\u0E2D\u0E40\u0E17\u0E47\u0E21\u0E17\u0E35\u0E48\u0E08\u0E30\u0E25\u0E1A\u0E2D\u0E2D\u0E01\u0E08\u0E32\u0E01\u0E23\u0E32\u0E22\u0E01\u0E32\u0E23\u0E17\u0E35\u0E48\u0E44\u0E21\u0E48\u0E16\u0E39\u0E01\u0E08\u0E31\u0E14\u0E40\u0E23\u0E35\u0E22\u0E07.",
    "allstacker.message.unstacked.removed": (pl) => `\xA7c\u0E25\u0E1A\xA7r %name \u0E2D\u0E2D\u0E01\u0E08\u0E32\u0E01\u0E23\u0E32\u0E22\u0E01\u0E32\u0E23\u0E17\u0E35\u0E48\u0E44\u0E21\u0E48\u0E16\u0E39\u0E01\u0E08\u0E31\u0E14\u0E40\u0E23\u0E35\u0E22\u0E07.`,
    // View Unstacked Items
    "allstacker.title.unstacked_items": (pl) => "\xA78\u0E23\u0E32\u0E22\u0E01\u0E32\u0E23\u0E17\u0E35\u0E48\u0E44\u0E21\u0E48\u0E16\u0E39\u0E01\u0E08\u0E31\u0E14\u0E40\u0E23\u0E35\u0E22\u0E07",
    "allstacker.body.unstacked_items": (pl) => "\u0E23\u0E32\u0E22\u0E01\u0E32\u0E23\u0E02\u0E2D\u0E07\u0E44\u0E2D\u0E40\u0E17\u0E47\u0E21\u0E17\u0E35\u0E48\u0E44\u0E21\u0E48\u0E16\u0E39\u0E01\u0E08\u0E31\u0E14\u0E40\u0E23\u0E35\u0E22\u0E07.",
    "allstacker.label.no_unstacked_items": (pl) => "\xA7c\u0E44\u0E21\u0E48\u0E1E\u0E1A\u0E23\u0E32\u0E22\u0E01\u0E32\u0E23\u0E17\u0E35\u0E48\u0E44\u0E21\u0E48\u0E16\u0E39\u0E01\u0E08\u0E31\u0E14\u0E40\u0E23\u0E35\u0E22\u0E07.",
    // Advanced Settings
    "allstacker.title.advanced_settings": (pl) => "\xA78\u0E01\u0E32\u0E23\u0E15\u0E31\u0E49\u0E07\u0E04\u0E48\u0E32\u0E02\u0E31\u0E49\u0E19\u0E2A\u0E39\u0E07",
    ...
    "allstacker.toggle.itemstack": (pl) => "\xA7c\u0E1B\u0E34\u0E14\xA77/\xA7a\u0E40\u0E1B\u0E34\u0E14 \xA7r\u0E01\u0E32\u0E23\u0E23\u0E27\u0E21\u0E44\u0E2D\u0E40\u0E17\u0E47\u0E21\xA7r",
    "allstacker.slider.radius_seeing": (pl) => "\xA77\u0E23\u0E30\u0E22\u0E30\u0E01\u0E32\u0E23\u0E21\u0E2D\u0E07\u0E40\u0E2B\u0E47\u0E19\u0E44\u0E2D\u0E40\u0E17\u0E47\u0E21\xA7r",
    "allstacker.slider.radius_combine": (pl) => "\xA77\u0E23\u0E30\u0E22\u0E30\u0E01\u0E32\u0E23\u0E23\u0E27\u0E21\u0E44\u0E2D\u0E40\u0E17\u0E47\u0E21\xA7r",
    "allstacker.textfield.display_text": (pl) => "\xA77\u0E02\u0E49\u0E2D\u0E04\u0E27\u0E32\u0E21\u0E41\u0E2A\u0E14\u0E07\u0E1C\u0E25\xA7r\n %%a\xA77 - \u0E41\u0E2A\u0E14\u0E07\u0E08\u0E33\u0E19\u0E27\u0E19\xA7r\n %%n \xA77- \u0E41\u0E2A\u0E14\u0E07\u0E0A\u0E37\u0E48\u0E2D\xA7r\n %%m \xA77- \u0E41\u0E2A\u0E14\u0E07\u0E19\u0E32\u0E17\u0E35\xA7r\n %%s \xA77- \u0E41\u0E2A\u0E14\u0E07\u0E27\u0E34\u0E19\u0E32\u0E17\u0E35\xA7r\n %%l \xA77- \u0E1A\u0E23\u0E23\u0E17\u0E31\u0E14\u0E43\u0E2B\u0E21\u0E48\xA7r",
    "allstacker.textfield.display_text.placeholder": (pl) => "\u0E1B\u0E23\u0E31\u0E1A\u0E41\u0E15\u0E48\u0E07\u0E02\u0E49\u0E2D\u0E04\u0E27\u0E32\u0E21\u0E17\u0E35\u0E48\u0E41\u0E2A\u0E14\u0E07\u0E2A\u0E33\u0E2B\u0E23\u0E31\u0E1A\u0E44\u0E2D\u0E40\u0E17\u0E47\u0E21\u0E17\u0E35\u0E48\u0E23\u0E27\u0E21\u0E01\u0E31\u0E19.",
    // Messages
    "allstacker.message.display_text.changed": (pl) => "\xA7a\u0E40\u0E1B\u0E25\u0E35\u0E48\u0E22\u0E19\u0E02\u0E49\u0E2D\u0E04\u0E27\u0E32\u0E21\u0E41\u0E2A\u0E14\u0E07\u0E1C\u0E25\u0E40\u0E1B\u0E47\u0E19: %value",
    "allstacker.message.radius_seeing.changed": (pl) => "\xA7a\u0E40\u0E1B\u0E25\u0E35\u0E48\u0E22\u0E19\u0E23\u0E30\u0E22\u0E30\u0E01\u0E32\u0E23\u0E21\u0E2D\u0E07\u0E40\u0E2B\u0E47\u0E19\u0E44\u0E2D\u0E40\u0E17\u0E47\u0E21\xA7r \u0E40\u0E1B\u0E47\u0E19: %value",
    "allstacker.message.radius_combine.changed": (pl) => "\xA7a\u0E40\u0E1B\u0E25\u0E35\u0E48\u0E22\u0E19\u0E23\u0E30\u0E22\u0E30\u0E01\u0E32\u0E23\u0E23\u0E27\u0E21\u0E44\u0E2D\u0E40\u0E17\u0E47\u0E21\xA7r \u0E40\u0E1B\u0E47\u0E19: %value",
    "allstacker.message.plugin.enabled": (pl) => "\xA7a\u0E40\u0E1B\u0E34\u0E14\u0E43\u0E0A\u0E49\u0E07\u0E32\u0E19\u0E1B\u0E25\u0E31\u0E4A\u0E01\u0E2D\u0E34\u0E19 ItemStacker \u0E41\u0E25\u0E49\u0E27!",
    "allstacker.message.plugin.disabled": (pl) => "\xA7a\u0E1B\u0E34\u0E14\u0E43\u0E0A\u0E49\u0E07\u0E32\u0E19\u0E1B\u0E25\u0E31\u0E4A\u0E01\u0E2D\u0E34\u0E19 ItemStacker \u0E41\u0E25\u0E49\u0E27!",
    // MobStacker Main Settings
    "allstacker.title.mobstacker": (pl) => "\xA78\u0E01\u0E32\u0E23\u0E15\u0E31\u0E49\u0E07\u0E04\u0E48\u0E32\u0E01\u0E32\u0E23\u0E23\u0E27\u0E21\u0E21\u0E2D\u0E1A",
    "allstacker.body.mobstacker": (pl) => "\u0E01\u0E33\u0E2B\u0E19\u0E14\u0E04\u0E48\u0E32\u0E1B\u0E25\u0E31\u0E4A\u0E01\u0E2D\u0E34\u0E19 Mob Stacker.",
    "allstacker.label.mobstacker.description": (pl) => "\xA7a\u0E2A\u0E32\u0E21\u0E32\u0E23\u0E16\xA77 \u0E40\u0E1E\u0E34\u0E48\u0E21, \u0E25\u0E1A, \u0E14\u0E39 \xA7b\u0E21\u0E2D\u0E1A\u0E17\u0E35\u0E48\u0E23\u0E27\u0E21\u0E01\u0E31\u0E19\xA7r.",
    "allstacker.button.mobstacker_settings": (pl) => "\u0E01\u0E32\u0E23\u0E15\u0E31\u0E49\u0E07\u0E04\u0E48\u0E32\u0E01\u0E32\u0E23\u0E23\u0E27\u0E21",
    "allstacker.label.mobstacker.advanced.description": (pl) => "\xA7a\u0E2A\u0E32\u0E21\u0E32\u0E23\u0E16\xA77 \xA72\u0E40\u0E1B\u0E34\u0E14\xA77/\xA7c\u0E1B\u0E34\u0E14\xA7r \u0E41\u0E25\u0E30\u0E1B\u0E23\u0E31\u0E1A\u0E23\u0E30\u0E22\u0E30\u0E01\u0E32\u0E23\u0E23\u0E27\u0E21, \u0E02\u0E49\u0E2D\u0E04\u0E27\u0E32\u0E21\u0E41\u0E2A\u0E14\u0E07\u0E1C\u0E25\u0E02\u0E2D\u0E07\u0E21\u0E2D\u0E1A\u0E17\u0E35\u0E48\u0E23\u0E27\u0E21\u0E01\u0E31\u0E19 \u0E41\u0E25\u0E30\u0E2D\u0E37\u0E48\u0E19\u0E46",
    // MobStacker Stacking Settings
    "allstacker.title.mob_stacking_settings": (pl) => "\xA78\u0E01\u0E32\u0E23\u0E15\u0E31\u0E49\u0E07\u0E04\u0E48\u0E32\u0E01\u0E32\u0E23\u0E23\u0E27\u0E21",
    ...
    // Add Stacked Mobs
    "allstacker.title.add_stacked_mobs": (pl) => "\xA78\u0E40\u0E1E\u0E34\u0E48\u0E21\u0E21\u0E2D\u0E1A\u0E17\u0E35\u0E48\u0E23\u0E27\u0E21\u0E44\u0E14\u0E49",
    "allstacker.body.add_stacked_mobs": (pl) => "\u0E40\u0E25\u0E37\u0E2D\u0E01\u0E21\u0E2D\u0E1A\u0E17\u0E35\u0E48\u0E04\u0E38\u0E13\u0E15\u0E49\u0E2D\u0E07\u0E01\u0E32\u0E23\u0E43\u0E2B\u0E49\u0E23\u0E27\u0E21\u0E44\u0E14\u0E49\u0E43\u0E19\u0E23\u0E30\u0E22\u0E30 10 \u0E1A\u0E25\u0E47\u0E2D\u0E01.",
    "allstacker.label.no_stackable_mobs": (pl) => "\xA7c\u0E44\u0E21\u0E48\u0E1E\u0E1A\u0E21\u0E2D\u0E1A\u0E17\u0E35\u0E48\u0E2A\u0E32\u0E21\u0E32\u0E23\u0E16\u0E23\u0E27\u0E21\u0E44\u0E14\u0E49\u0E43\u0E19\u0E23\u0E30\u0E22\u0E30\u0E19\u0E35\u0E49.",
    "allstacker.message.mob.added": (pl) => "\xA7a\u0E40\u0E1E\u0E34\u0E48\u0E21 %name \u0E25\u0E07\u0E43\u0E19\u0E23\u0E32\u0E22\u0E01\u0E32\u0E23\u0E21\u0E2D\u0E1A\u0E17\u0E35\u0E48\u0E23\u0E27\u0E21\u0E44\u0E14\u0E49.",
    // Remove Stacked Mobs
    "allstacker.title.remove_stacked_mobs": (pl) => "\xA78\u0E25\u0E1A\u0E21\u0E2D\u0E1A\u0E17\u0E35\u0E48\u0E23\u0E27\u0E21\u0E44\u0E14\u0E49",
    "allstacker.body.remove_stacked_mobs": (pl) => "\u0E40\u0E25\u0E37\u0E2D\u0E01\u0E21\u0E2D\u0E1A\u0E17\u0E35\u0E48\u0E04\u0E38\u0E13\u0E15\u0E49\u0E2D\u0E07\u0E01\u0E32\u0E23\u0E25\u0E1A\u0E2D\u0E2D\u0E01\u0E08\u0E32\u0E01\u0E01\u0E32\u0E23\u0E23\u0E27\u0E21.",
    "allstacker.label.no_stacked_mobs": (pl) => "\xA7c\u0E44\u0E21\u0E48\u0E1E\u0E1A\u0E21\u0E2D\u0E1A\u0E17\u0E35\u0E48\u0E23\u0E27\u0E21\u0E44\u0E14\u0E49.",
    "allstacker.message.mob.removed": (pl) => "\xA7a\u0E25\u0E1A %name \u0E2D\u0E2D\u0E01\u0E08\u0E32\u0E01\u0E23\u0E32\u0E22\u0E01\u0E32\u0E23\u0E21\u0E2D\u0E1A\u0E17\u0E35\u0E48\u0E23\u0E27\u0E21\u0E44\u0E14\u0E49.",
    // View Stacked Mobs
    "allstacker.title.view_stacked_mobs": (pl) => "\xA78\u0E14\u0E39\u0E21\u0E2D\u0E1A\u0E17\u0E35\u0E48\u0E23\u0E27\u0E21\u0E44\u0E14\u0E49",
    "allstacker.body.view_stacked_mobs": (pl) => "\u0E23\u0E32\u0E22\u0E01\u0E32\u0E23\u0E21\u0E2D\u0E1A\u0E17\u0E35\u0E48\u0E2A\u0E32\u0E21\u0E32\u0E23\u0E16\u0E23\u0E27\u0E21\u0E44\u0E14\u0E49\u0E43\u0E19\u0E1B\u0E31\u0E08\u0E08\u0E38\u0E1A\u0E31\u0E19.",
    // MobStacker Advanced Settings
    "allstacker.title.mob_advanced_settings": (pl) => "\xA78\u0E01\u0E32\u0E23\u0E15\u0E31\u0E49\u0E07\u0E04\u0E48\u0E32\u0E02\u0E31\u0E49\u0E19\u0E2A\u0E39\u0E07",
    "allstacker.body.mob_advanced_settings": (pl) => "",
    "allstacker.label.mob_advanced.description": (pl) => "\u0E01\u0E33\u0E2B\u0E19\u0E14\u0E04\u0E48\u0E32\u0E01\u0E32\u0E23\u0E15\u0E31\u0E49\u0E07\u0E04\u0E48\u0E32\u0E02\u0E31\u0E49\u0E19\u0E2A\u0E39\u0E07\u0E2A\u0E33\u0E2B\u0E23\u0E31\u0E1A\u0E1B\u0E25\u0E31\u0E4A\u0E01\u0E2D\u0E34\u0E19 Mob Stacker.",
    "allstacker.toggle.mobstacker": (pl) => "\xA7c\u0E1B\u0E34\u0E14\xA77/\xA7a\u0E40\u0E1B\u0E34\u0E14\xA7f MobStacker",
    "allstacker.dropdown.mob_death_mode": (pl) => "\xA77\u0E42\u0E2B\u0E21\u0E14\u0E01\u0E32\u0E23\u0E15\u0E32\u0E22\u0E02\u0E2D\u0E07\u0E21\u0E2D\u0E1A",
    "allstacker.slider.radius_stacking": (pl) => "\xA77\u0E23\u0E30\u0E22\u0E30\u0E01\u0E32\u0E23\u0E23\u0E27\u0E21\u0E21\u0E2D\u0E1A\u0E43\u0E01\u0E25\u0E49\u0E40\u0E04\u0E35\u0E22\u0E07",
    "allstacker.textfield.mob_display_text": (pl) => "\xA77\u0E02\u0E49\u0E2D\u0E04\u0E27\u0E32\u0E21\u0E41\u0E2A\u0E14\u0E07\u0E1C\u0E25\n \xA7r%%a \xA77- \u0E41\u0E2A\u0E14\u0E07\u0E08\u0E33\u0E19\u0E27\u0E19\n\xA7r %%n \xA77- \u0E41\u0E2A\u0E14\u0E07\u0E0A\u0E37\u0E48\u0E2D\n \xA7r%%l \xA77- \u0E1A\u0E23\u0E23\u0E17\u0E31\u0E14\u0E43\u0E2B\u0E21\u0E48",
    "allstacker.textfield.mob_display_text.placeholder": (pl) => "\u0E43\u0E2A\u0E48\u0E02\u0E49\u0E2D\u0E04\u0E27\u0E32\u0E21\u0E41\u0E2A\u0E14\u0E07\u0E1C\u0E25\u0E2A\u0E33\u0E2B\u0E23\u0E31\u0E1A\u0E21\u0E2D\u0E1A\u0E17\u0E35\u0E48\u0E23\u0E27\u0E21\u0E01\u0E31\u0E19",
    "allstacker.button.save_changes": (pl) => "\xA78\u0E1A\u0E31\u0E19\u0E17\u0E36\u0E01\u0E01\u0E32\u0E23\u0E40\u0E1B\u0E25\u0E35\u0E48\u0E22\u0E19\u0E41\u0E1B\u0E25\u0E07",
    // MobStacker Messages
    "allstacker.message.mob_death_mode.changed": (pl) => "\xA7a\u0E2D\u0E31\u0E1B\u0E40\u0E14\u0E15\u0E42\u0E2B\u0E21\u0E14\u0E01\u0E32\u0E23\u0E15\u0E32\u0E22\u0E02\u0E2D\u0E07\u0E21\u0E2D\u0E1A\u0E40\u0E1B\u0E47\u0E19 %value.",
    "allstacker.message.stacking_radius.changed": (pl) => "\xA7a\u0E2D\u0E31\u0E1B\u0E40\u0E14\u0E15\u0E23\u0E30\u0E22\u0E30\u0E01\u0E32\u0E23\u0E23\u0E27\u0E21\u0E40\u0E1B\u0E47\u0E19 %value \u0E1A\u0E25\u0E47\u0E2D\u0E01.",
    "allstacker.message.mob_display_text.changed": (pl) => "\xA7a\u0E2D\u0E31\u0E1B\u0E40\u0E14\u0E15\u0E02\u0E49\u0E2D\u0E04\u0E27\u0E32\u0E21\u0E41\u0E2A\u0E14\u0E07\u0E1C\u0E25\u0E40\u0E1B\u0E47\u0E19: %value",
    "allstacker.message.mobstacker.enabled": (pl) => "\xA7a\u0E1B\u0E25\u0E31\u0E4A\u0E01\u0E2D\u0E34\u0E19 Mob Stacker \u0E40\u0E1B\u0E34\u0E14\u0E43\u0E0A\u0E49\u0E07\u0E32\u0E19\u0E41\u0E25\u0E49\u0E27.",
    "allstacker.message.mobstacker.disabled": (pl) => "\xA7a\u0E1B\u0E25\u0E31\u0E4A\u0E01\u0E2D\u0E34\u0E19 Mob Stacker \u0E1B\u0E34\u0E14\u0E43\u0E0A\u0E49\u0E07\u0E32\u0E19\u0E41\u0E25\u0E49\u0E27."
  });
...
var ConfigMenu = class extends PluginBase {
  ...
  showConfig(pl) {
    ...
    let list = [
      "\xA78All Stackers Settings",
      "Hello, \xA7e${pl.name}\xA7r!\n\nThis is the configuration menu.\nYou can manage settings here.",
      "\xA73Langguage",
      "\xA78Language Settings",
      "Select your preferred language.",
      `\xA77[All Stacker] \xA7rLanguage set to \xA7aEnglish\xA7r.`,
      `\xA77[All Stacker] \xA7rLanguage set to \xA7a\u0E44\u0E17\u0E22\xA7r.`,
      `\xA7cBack`,
      `\xA77Plugins \xA77(\xA7c${pluginSettingList.length}\xA77)`,
      `\xA72Enabled\xA7r`,
      `\xA7cDisabled\xA7r`
    ];
...
var ItemStacker = class extends PluginBase {
      ...
      "Stacking Settings": () => {
        ...
        unStackPage.addButton(LanguageContext2.getTranslation("allstacker.button.add_unstacked", pl), "textures/ui/icon_book_writable", () => {
          ...
          Object.entries(itemList).forEach(([name, item]) => {
            if (!item || !item.typeId)
              return;
            if (this.config.ItemStackConfig.get("UnStackItem")?.includes(item.typeId))
              return;
  ...
  onItemRemoved(ev) {
    if (ev.removedEntity.typeId !== "minecraft:item" || ev.removedEntity.hasTag("fakeItem") || this.config.ItemListStack.has(ev.removedEntity))
      return;
  ...
  onLoad(ev) {
    ...
    this.runJobs();
    KXEvents.on(this, "after:entitySpawn", (ev2) => {
      this.onItemSpawned(ev2);
    });
    KXEvents.on(this, "before:entityRemove", (ev2) => {
      this.onItemRemoved(ev2);
    });
...
// scripts/kisux3/plugins/MobStacker/index.ts
import { EntityDamageCause, EntityEquippableComponent, EntityProjectileComponent, EquipmentSlot, system as system7, world as world10 } from "@minecraft/server";

// scripts/kisux3/plugins/MobStacker/services/utils.ts
  ...
  new Promise(async (resolve) => {
    try {
      const allEntities = getAllEntities((en) => {
        if (!config.ResetEntities.has(en) && [...config.MobStackConfig.get("StackMob") || []].some((b) => b === en.typeId) && en.location)
          return true;
        return false;
      });
...
function getEntitiesNearBy(dimension, en, config) {
  ...
  const allEn = dimension.getEntities({ location: en.location, maxDistance: radiusStacking, type: en.typeId }).filter((x) => x.id !== en.id).filter((x) => !config.ResetEntities.has(x)).filter((x) => x.hasComponent("is_baby") == en.hasComponent("is_baby")).filter((x) => !x.hasComponent("is_tamed")).filter((x) => {
    ...
    if (isHasStackEn && isHasStackTarget)
      return true;
    if (!(isHasStackEn && isHasStackTarget))
      return true;
    return false;
...
var MobStacker = class extends PluginBase {
  ...
  onLoad(ev) {
    this.initializeConfig();
    this.runJobs();
    KXEvents.on(this, "after:entityDie", (ev2) => {
      this.onEntityDie(ev2);
    });
    KXEvents.on(this, "before:playerInteractWithEntity", (ev2) => {
      this.onEntityInteract(ev2);
    });
    KXEvents.on(this, "before:entityRemove", (ev2) => {
      this.onXpDrop(ev2);
    });
  ...
  onXpDrop(ev) {
    ...
    system7.run(() => {
      if (this.config.Xp_Queue.has(RemovedEntityData.id)) {
        ...
        const xp_orb = world10.getDimension(RemovedEntityData.dimension).getEntities({
        ...
        for (let i = 0; i < xpData; i++) {
          xp_orb.forEach((orb) => {
            if (orb.isValid) {
              const orbSpawn = world10.getDimension(RemovedEntityData.dimension).spawnEntity("minecraft:xp_orb", orb.location);
            }
          });
        }
      }
  ...
  initializeConfig() {
    this.config = this.getConfig();
    this.config.MobStackConfig = new JsonDatabase("MobStackConfig", world10);
...
// scripts/kisux3/configs/PluginLoader.ts
var PluginLoader = [
  {
    name: "ConfigMenu",
